"""This python script is used to generate a list of detecotr banks with 
   a bank number, a theta angle, a list of detector IDs and a phi angle.

   The bank number is arbitary, while the detector ID corresponds to the
...
def write_line(bank, angle, detector_ids, height_angle):
    f.write(str(bank) + '\t\t' + str(angle) + '\t' + str(detector_ids).replace(' ', '').replace('[', '').replace(']', ' ').ljust(16) + '\t' + str(height_angle) + '\n')


def find_average_angle_for_bank_and_slice(detectors, number_of_detectors_middle, number_of_detectors_upper_lower):
    """ Finds the centre theta angle for the detector boxes. Then removes
        these detectors from the list.
    """
    
    angles_for_middle_bank = detectors[0:number_of_detectors_middle]
    middle_bank_angle = sum(angles_for_middle_bank)/len(angles_for_middle_bank)
    
    angles_for_upper_lower_bank = detectors[0:number_of_detectors_upper_lower]
    if len(angles_for_upper_lower_bank) > 0:
        upper_lower_bank_angle = sum(angles_for_upper_lower_bank)/len(angles_for_upper_lower_bank)
    else:
        upper_lower_bank_angle = 0
    
    detectors = detectors[max(number_of_detectors_middle, number_of_detectors_upper_lower):]
    return middle_bank_angle, upper_lower_bank_angle, detectors
...
if __name__ == '__main__':
    ...
    f.write('Bank# \tTheta \tDetector ID \t\tPhi \n')
    ...
    for i in range(number_of_banks):
        """There are 4 different regimes for the theta angle, 
            ...
            if they exist for upper, lower and/or middle banks.
        """
        ...
        if angle < 17:
            ...
            middle_bank_angle, upper_lower_bank_angle, detectors = find_average_angle_for_bank_and_slice(detectors, number_of_detectors_middle, number_of_detectors_upper_lower)
            write_line(bank, middle_bank_angle, [d_id, d_id + 1, d_id + 2, d_id + 3], middle_angle)
            bank += 1
            d_id += 4
        elif 17 < angle < 44:
            number_of_detectors_middle = 4
            ...
            # Upper
            write_line(bank, upper_lower_bank_angle, [d_id, d_id + 3, d_id + 6], upper_angle)
            bank += 1
            d_id += 1
            # Middle
            write_line(bank, middle_bank_angle, [d_id, d_id + 3, d_id + 6, d_id + 8], middle_angle)
            bank += 1
            d_id += 1
            # Lower
            write_line(bank, upper_lower_bank_angle, [d_id, d_id + 3, d_id + 6], lower_angle)
            bank += 1
            d_id += 1
            ...
            # Upper
            write_line(bank, upper_lower_bank_angle, [d_id, d_id + 3, d_id + 6, d_id + 9], upper_angle)
            bank += 1
            d_id += 1
            # Middle
            write_line(bank, middle_bank_angle, [d_id, d_id + 3, d_id + 6, d_id + 9], middle_angle)
            bank += 1
            d_id += 1
            # Lower
            write_line(bank, upper_lower_bank_angle, [d_id, d_id + 3, d_id + 6, d_id + 9], lower_angle)
            bank += 1
            d_id += 1
            ...
            d_id += 1
            # Middle
            write_line(bank, middle_bank_angle, [d_id, d_id + 3, d_id + 6], middle_angle)
            bank += 1
            d_id += 1
            # Lower
            write_line(bank, upper_lower_bank_angle, [d_id, d_id + 3, d_id + 6], lower_angle)
