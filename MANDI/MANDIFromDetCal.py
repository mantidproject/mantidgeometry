...
def writeToFile(xmlOut,append):
    ...
    today = date.today()
    filename = "%s_Definition_%4i-%02i-%02i.xml" % ("MANDI",today.year,today.month,today.day)
...
def writeToFile2(xmlOut,append):
    """Writes the XML to a file with the prescriptive name"""
    today = date.today()
    filename = "%s_Parameters_%4i-%02i-%02i.xml" % ("MANDI",today.year,today.month,today.day)
    f1=open(filename, append)
    f1.write(xmlOut)
    f1.write("\n")
...
def makeMantidParameters(t0):
    ...
    sxml = """<?xml version='1.0' encoding='UTF-8'?>
<parameter-file instrument = "MANDI" valid-from   ="%s" valid-to     ="2100-12-31 23:59:59">
...
def makeMantidGeometryPanel(det_num, widX, widY):
    """ Generate XML code to make a bit of XML for use in the
    Mantid XML geometry.
...
<type name="panel%d" is="rectangular_detector" type="pixel"
    xpixels="256" xstart="%f" xstep="%f"
    ypixels="256" ystart="%f" ystep="%f" >
  <properties/>
</type> """ %(det_num, startX, deltaX, startY, deltaY)
...
def makeMantidGeometryCode(det_num, name, idstart, radius, theta_center_rad, phi_center_rad, phi_rad, chi_rad, omega_rad):
    """ Generate XML code to make a bit of XML for use in the
    Mantid XML geometry.
    ...
    sxml = """<component type="panel%d" idstart="%d" idfillbyfirst="y" idstepbyrow="256">
<location r="%f" t="%f" p="%f" name="%s" rot="%f" axis-x="0" axis-y="1" axis-z="0">
  <rot val="%f">
    <rot val="%f" axis-x="0" axis-y="1" axis-z="0" />
  </rot>
</location>
</component> """ % (det_num, idstart, r, theta_center, phi_center, name, omega, chi, phi)
...
##    #   i.e.: the detector rotation, which is -45 degrees for MANDI
...
for row in range(len(open(detCalFile).readlines())):
    ...
    if flag == "5": #Is it in use?
        ...
        writeToFile(makeMantidGeometryPanel(det_num, widX, widY), "a")
...
maths.addEquation("real_phi = phi", comment="MANDI Ambient Goniometer - taken from CV info file.")
